...
exp(cbind(OR = coef(M_boomer), confint(M_boomer)))
...
#to look at the stargazer, try the one above, which uses "text." The one below only appears
...
ggplot(blackfem_ed_genY, aes(x = factor(1:5), y = probability, group = 1)) +
  geom_line() + geom_line(data = blackfem_ed_boomer, aes(y = probability)) +
  labs(x = "Education", y = "Probability of voting", title = "Millennial vs Boomer") +    scale_x_discrete(labels = c("Below high school", "High school", "Some post-high", 
                                "Bachelor", "Graduate")) +
  geom_ribbon(data=blackfem_ed_genY,aes(ymin=lb,ymax=ub),alpha=0.3) +
...
#Now we'd like to compare education levels betweeen Millennials and Babyboomers.
#Do education levels have the same predictive power across these two generations? 

genY_ed1 <- predicted_fit(-1.42663, 0.27571, 0.10578, 1.08370, 0.97081, 0.63016, 
                                0.19622, 1, 1, 0, 1, 0, 3, 0.13361, 0.23487, 0.24849, 
                                0.39274, 0.35245, 0.07876)

genY_ed2 <- predicted_fit(-1.42663, 0.27571, 0.10578, 1.08370, 0.97081, 0.63016, 
                                0.19622, 2, 1, 0, 1, 0, 3, 0.13361, 0.23487, 0.24849, 
                                0.39274, 0.35245, 0.07876)

genY_ed3 <- predicted_fit(-1.42663, 0.27571, 0.10578, 1.08370, 0.97081, 0.63016, 
                                0.19622, 3, 1, 0, 1, 0, 3, 0.13361, 0.23487, 0.24849, 
                                0.39274, 0.35245, 0.07876)

genY_ed4 <- predicted_fit(-1.42663, 0.27571, 0.10578, 1.08370, 0.97081, 0.63016, 
                                0.19622, 4, 1, 0, 1, 0, 3, 0.13361, 0.23487, 0.24849, 
                                0.39274, 0.35245, 0.07876)

genY_ed5 <- predicted_fit(-1.42663, 0.27571, 0.10578, 1.08370, 0.97081, 0.63016, 
                                0.19622, 5, 1, 0, 1, 0, 3, 0.13361, 0.23487, 0.24849, 
                                0.39274, 0.35245, 0.07876)

boomer_ed1 <- predicted_fit(-1.49411, 0.32012, 0.18731, 2.67234, -0.73006, -0.12489,
                                  0.17901, 1, 1, 0, 1, 0, 3, 0.12001, 0.25347, 0.25373,
                                  0.35665, 0.39013, 0.06914)

boomer_ed2 <- predicted_fit(-1.49411, 0.32012, 0.18731, 2.67234, -0.73006, -0.12489,
                                  0.17901, 2, 1, 0, 1, 0, 3, 0.12001, 0.25347, 0.25373,
                                  0.35665, 0.39013, 0.06914)

boomer_ed3 <- predicted_fit(-1.49411, 0.32012, 0.18731, 2.67234, -0.73006, -0.12489,
                                  0.17901, 3, 1, 0, 1, 0, 3, 0.12001, 0.25347, 0.25373,
                                  0.35665, 0.39013, 0.06914)

boomer_ed4 <- predicted_fit(-1.49411, 0.32012, 0.18731, 2.67234, -0.73006, -0.12489,
                                  0.17901, 4, 1, 0, 1, 0, 3, 0.12001, 0.25347, 0.25373,
                                  0.35665, 0.39013, 0.06914)

boomer_ed5 <- predicted_fit(-1.49411, 0.32012, 0.18731, 2.67234, -0.73006, -0.12489,
                                  0.17901, 5, 1, 0, 1, 0, 3, 0.12001, 0.25347, 0.25373,
                                  0.35665, 0.39013, 0.06914)
...
library(dplyr)
ed_genY <- rbind(black_genY_ed1, black_genY_ed2, black_genY_ed3, black_genY_ed4,
                   black_genY_ed5)  %>% as.data.frame()
ed_boomer <- rbind(black_boomer_ed1, black_boomer_ed2, black_boomer_ed3,
                   black_boomer_ed4, black_boomer_ed5) %>% as.data.frame()

colnames(ed_genY) <- c("lb", "probability", "ub")
colnames(ed_boomer) <- c("lb", "probability", "ub")
rownames(ed_genY) <- c("Below high school", "High school", "Some post-high", 
                                "Bachelor", "Graduate")
rownames(ed_boomer) <- c("Below high school", "High school", "Some post-high", 
                                "Bachelor", "Graduate")
...
ggplot(ed_genY, aes(x = factor(1:5), y = probability, group = 1)) +
  geom_line() + geom_line(data = ed_boomer, aes(y = probability)) +
  labs(x = "Education", y = "Probability of voting", title = "Millennial vs Boomer") +    scale_x_discrete(labels = c("Below high school", "High school", "Some post-high", 
                                "Bachelor", "Graduate")) +
  geom_ribbon(data=ed_genY,aes(ymin=lb,ymax=ub),alpha=0.3) +
  geom_ribbon(data=ed_boomer,aes(ymin=lb,ymax=ub),alpha=0.3) 
